...
function* loginUser(action) {
	try {
		...
		if (error.response.status === 401) {
			// if user isn't in the database or if username / password don't match  db
...
function* logoutUser(action) {
	try {
		...
		// now that the session has ended on the server remove the client-side user object to let
